...
import {
  BaseProvider,
  Event,
} from '@ethersproject/providers/src.ts/base-provider';
...
export class OpenSigner extends Signer implements TypedDataSigner {
  ...
  async sendUserOperation(
    userOperation: Deferrable<UserOperation>,
  ): Promise<string> {
    ...
    const tx: Deferrable<UserOperation> = await resolveProperties(
      userOperation,
    );
    ...
    return this.provider.send('eth_sendUserOperation', [hexUserOp]).then(
      (hash) => {
        return hash;
      },
      (error) => {
